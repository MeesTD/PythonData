...
with open('./csv_bestanden/hotel_bookings.csv', 'r') as f:
    ...
    mijncursor.execute(
        'CREATE TABLE hotel_booking(id INT not null AUTO_INCREMENT, PRIMARY KEY (id), hotel VARCHAR(30), arrival_date_year INT, arrival_date_month VARCHAR(30), arrival_date_day_of_month INT, meal VARCHAR(30), adr DECIMAL, date DATE);')
    ...
    for row in data1:
        ...
        if i != 0 and (float(adr) >= 80 and float(adr) <= 250):
            ...
            date))  # execute(SQL statement, (hier de waarden voor de plekken %s en op volgorde)))
